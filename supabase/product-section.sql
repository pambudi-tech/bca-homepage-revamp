...
-- CTA button text sudah dipindah ke i18n messages (`t("ctaLabel")` di
-- `ProductSection.tsx`), jadi kolom ini tidak pernah dibaca — dihapus dari
-- instalasi yang masih punya kolomnya.
alter table public.product_categories drop column if exists cta_label;
alter table public.product_categories drop column if exists cta_label_en;
alter table public.product_categories drop column if exists cta_label_zh;
...
insert into public.product_categories (key, label, sort_order, is_default) values
  ('Simpanan',     'Simpanan',     1, false),
  ('Kartu Kredit', 'Kartu Kredit', 2, true),
  ('Pinjaman',     'Pinjaman',     3, false),
  ('Investasi',    'Investasi',    4, false),
  ('Asuransi',     'Asuransi',     5, false)
